...
export class MyarcFDC implements FDC, DSRCard, MemoryMappedCard {
    ...
    constructor(
        ...
        this.wd177x = new WD177x(diskDrives);
    }
    ...
    public writeCruBit(bit: number, value: boolean) {
        ...
        switch (bit) {
            ...
            case 3:
                ...
                break;
            case 4:
                this.wd177x.setDrive(1, value);
                break;
            case 5:
                this.wd177x.setDrive(2 , value);
                break;
            case 6:
                this.wd177x.setDrive(3 , value);
                break;
            case 7:
                this.wd177x.setDrive(4 , value);
                break;
        }
    }
    ...
    public readCruBit(bit: number): boolean {
        ...
        switch (bit) {
            case 0:
                // true means not busy
                const busy = this.wd177x.isBusy();
                this.log.debug("Read busy: " + busy);
                return !busy;
            case 1:
                // true means ready to process data
                const ready = this.wd177x.isReadyForData();
                this.log.debug("Read ready for data: " + ready);
                return ready;
            case 2:
                return true;
            case 3:
    ...
    readMemoryMapped(addr: number, cpu: CPU): number {
        if (addr < 0x5000) {
            ...
            switch (addr) {
                case 0x5f00:
                    byte = this.wd177x.getStatus();
                    break;
                case 0x5f02:
                    byte = this.wd177x.getTrack();
                    break;
                case 0x5f04:
                    byte = this.wd177x.getSector();
                    break;
                case 0x5f06:
                    byte = this.wd177x.getData();
                    break;
            }
            return byte;
        } else {
        ...
        if (addr < 0x5000) {
            ...
            switch (addr) {
                case 0x5f00:
                    ...
                    this.wd177x.setCommand(byte);
                    break;
                case 0x5f02:
                    this.wd177x.setTrack(byte);
                    break;
                case 0x5f04:
                    this.wd177x.setSector(byte);
                    break;
                case 0x5f06:
                    this.wd177x.setData(byte);
                    break;
            }
        }
    }
    ...
    getState(): any {
        return {
            ...
            wd1771: this.wd177x.getState()
        };
    }

    restoreState(state: any): void {
        this.doubleDensity = state.doubleDensity;
        this.wd177x.restoreState(state.wd1771);
    }
}
